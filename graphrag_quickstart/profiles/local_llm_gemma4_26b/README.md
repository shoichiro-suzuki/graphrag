# GraphRAG Profile: local_llm_gemma4_26b
...
| 用途 | model id | 接続先 |
|---|---|---|
| ドキュメント解析 | `local_completion_model` / `gemma4:26b-a4b-it-qat` | `https://10.116.130.134:8443/v1` |
...
- ローカル解析モデル: `gemma4:26b-a4b-it-qat`
...
graphrag_quickstart\.venv\Scripts\python.exe -m graphrag index --root graphrag_quickstart\profiles\local_llm_gemma4_26b
``

本番的に回す前に、まず `--dry-run --skip-validation` で設定読み込みを確認してください。

``powershell
graphrag_quickstart\.venv\Scripts\python.exe -m graphrag index --root graphrag_quickstart\profiles\local_llm_gemma4_26b --dry-run --skip-validation
``
