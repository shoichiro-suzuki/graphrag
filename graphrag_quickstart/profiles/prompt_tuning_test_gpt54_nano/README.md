# GraphRAG Profile: prompt_tuning_test_gpt54_nano
...
- 本 profile は `prompt_tuning_test_gpt54_mini` を複製して作成した。`prompts/` および `_prompt_tune_output_no_discover/` は複製元（gpt-5.4-mini でチューニング済み）の内容をそのまま引き継いでおり、gpt-5.4-nano 向けの再チューニングは未実施
- gpt-5.4-nano 向けに prompt tuning をやり直す場合は、下記「実行例」を参照し、出力先フォルダ名を条件に合わせて変更すること
...
- Index model: `gpt-5.4-nano`
- Query model: `gpt-5.4-mini`（`query_completion_model` は既定のまま維持）
- Prompt tuning output: `.\_prompt_tune_output_no_discover`（gpt-5.4-mini でのチューニング結果を暫定流用）
...
- 例: `.\_prompt_tune_output_gpt54_nano_no_discover`
