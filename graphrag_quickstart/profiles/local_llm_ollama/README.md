# GraphRAG Profile: local_llm_ollama
...
| 用途 | model id | 接続先 |
|---|---|---|
| ドキュメント解析 | `local_completion_model` | `https://10.116.130.134:8443/v1` |
...
`local_completion_model` は OpenAI 互換 API として呼びます。社内ゲートウェイは自己署名 TLS のため、profile では `ssl_verify: false` を明示しています。ローカルサーバーの上限に合わせ、`concurrent_requests: 3` に抑えています。
...
graphrag_quickstart\.venv\Scripts\python.exe -m graphrag index --root graphrag_quickstart\profiles\local_llm_ollama
```

本番的に回す前に、まず `--dry-run --skip-validation` で設定読み込みを確認してください。

```powershell
graphrag_quickstart\.venv\Scripts\python.exe -m graphrag index --root graphrag_quickstart\profiles\local_llm_ollama --dry-run --skip-validation
```

## 注意

- `gpt-oss:20b` は reasoning 系なので、`extra_body.think: false` を付けて本文をクリーンに保ちます。
